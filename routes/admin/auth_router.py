...
def admin_login(req_data:LoginRequest, request:Request):
    try:
        db_user = AdminRepo.fetch_by_email(req_data.email)
        role = {}
...
async def update_user_password(req:AdminChangePassword, request:Request, token:str=Depends(auth_bearer)):
    ...
    current_user = AdminRepo.fetch_by_id(user_id)
    print("CURR USER", current_user, )
    if is_valid_password(req.current_password, current_user.get("password")):
        ...
        upd_res:AdminResult = await AdminRepo.change_password(user_id, pwd_hashed)
        if upd_res.get("data"):
