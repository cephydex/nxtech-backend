...
class VehicleRepo:
    ...
    q_builder = QueryBuilder(model=None).table("vehicles")
    ...
    async def delete(id: str):
        result = Vehicle.find(id).delete()

        return result


    def fetch_all_brands() -> List:
        result =  Vehicle.all(["id", "brand"])
        
        return result.serialize()


    def fetch_all_brand_models(id) -> List:
        result =  Vehicle.where("id", id).first(["models"])
